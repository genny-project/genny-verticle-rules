...
//			        .forEach(data -> log.info(data.questions.size()));
	...
	public static BaseEntity processAttribute(BaseEntityUtils beUtils, String targetCode, Tuple2<String,String> attributeCodeValue, String importAttributeCode,String matchAttributeCode,String prefixFilter,String linkAttributeCode, List<Answer> answers)
	{
			...
			if (foundMsg != null) {
				...
				if (foundMsg.getItems()!=null) {
					if (foundMsg.getItems().length > 0) {
						...
						Answer linkAnswer = new Answer(beUtils.getGennyToken().getUserCode(),targetCode,linkAttributeCode,linkValue);
	...
	public static BaseEntity linkToFind(BaseEntityUtils beUtils, String targetCode, Tuple2<String,String> attributeCodeValue, String importAttributeCode,String matchAttributeCode,String prefixFilter,String linkAttributeCode, List<Answer> answers)
	{
		if (attributeCodeValue._1.startsWith(importAttributeCode)) {
			QDataBaseEntityMessage foundMsg = QwandaUtils.findBaseEntityByAttributeCodeLikeValue(beUtils.getServiceToken().getRealm(), beUtils.getServiceToken().getToken(),
			matchAttributeCode, attributeCodeValue._2);
			...
			if (foundMsg != null) {
				...
				if (foundMsg.getItems()!=null) {
					if (foundMsg.getItems().length > 0) {
						...
						System.out.println("Setting up link for result "+linkAttributeCode+" with value "+linkValue);
						Answer linkAnswer = new Answer(beUtils.getGennyToken().getUserCode(),result.getCode(),linkAttributeCode,linkValue);
						beUtils.saveAnswer(linkAnswer);
						System.out.println("Set up "+linkAttributeCode+" with value "+matchAttributeCode+":"+attributeCodeValue._2);
						return result;
						} else {
							System.out.println("Could not Set up "+linkAttributeCode+" with value "+matchAttributeCode+":"+attributeCodeValue._2);
						}
					}
				}
			} else {
